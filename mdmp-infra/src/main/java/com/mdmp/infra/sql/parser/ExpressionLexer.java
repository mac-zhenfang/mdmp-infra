    ...
    public Token<?> scan(boolean analyse) {
        ...
        for (;; this.nextChar()) {
            ...
            if (analyse) {
                ...
                if (this.peek == '\n') {
                    throw new MDMPException("Aviator doesn't support multi-lines expression at "
        ...
        if (Character.isDigit(this.peek) || this.peek == '.') {
            ...
            do {
                ...
                if (this.peek == '.') {
                    if (scientificNotation) {
                        throw new MDMPException("Illegal number " + sb + " at "
                                + this.iterator.getIndex());
                    }
                    if (hasDot) {
                        throw new MDMPException("Illegal Number " + sb + " at "
            ...
            if (isBigDecimal) {
                ...
                value = new BigDecimal(lexeme, MathContext.DECIMAL128);
            }
            else if (isBigInt) {
                String lexeme = this.getBigNumberLexeme(sb);
        ...
        if (this.peek == '"' || this.peek == '\'') {
            ...
            while ((this.peek = this.iterator.next()) != left) {
                if (this.peek == CharacterIterator.DONE) {
                    throw new MDMPException("Illegal String " + sb + " at " + startIndex);
                }
                else {
                    sb.append(this.peek);
                }
