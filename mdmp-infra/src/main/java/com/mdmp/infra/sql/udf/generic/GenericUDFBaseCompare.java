...
import com.mdmp.common.exception.UDFArgumentTypeException;
...
/*    for (int i = 0; i < arguments.length; i++) {
    ...
    if (arguments[0].getClass().isInstance(String.class) &&
    ...
    } else if (arguments[0] instanceof Byte &&
      ...
      boi1 = (Boolean ) arguments[1];
    	 ...
    	 // TODO type convert
      /*TypeInfo oiTypeInfo0 = TypeInfoUtils.getTypeInfoFrom (arguments[0]);
      TypeInfo oiTypeInfo1 = TypeInfoUtils.getTypeInfoFrom (arguments[1]);

      if (oiTypeInfo0 != oiTypeInfo1) {
        compareType = CompareType.NEED_CONVERT;
        compareOI = TypeInfoUtils.getStandardWritable FromTypeInfo(
            TypeInfoFactory.doubleTypeInfo);
        converter0 =  Converters.getConverter(arguments[0], compareOI);
        converter1 =  Converters.getConverter(arguments[1], compareOI);
      } else {
