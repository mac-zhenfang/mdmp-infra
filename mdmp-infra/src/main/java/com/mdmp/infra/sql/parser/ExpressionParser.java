...
public class ExpressionParser {
	...
	public void ternary() {
		this.join();
		if (this.lookhead == null || this.expectLexeme(":")
				|| this.expectLexeme(",")) {
			return;
		}
	...
	public void factor() {
		...
		if (this.expectLexeme("(")) {
			...
			this.parenDepth--;
				...
				|| this.lookhead.getType() == TokenType.Variable
				|| this.lookhead == Variable.TRUE
				|| this.lookhead == Variable.FALSE
				|| this.isOPVariable(this.lookhead)) {
			...
			if (this.lookhead.getType() == TokenType.Char) {
				...
				// make it as variable
				this.lookhead = new Variable(charToken.getLexeme(),
						charToken.getStartIndex());
			}
			this.move(true);
			// function
	...
	private void array() {
		...
		if (RESERVED_WORDS.contains(this.prevToken.getLexeme())) {
			throw new MDMPException(this.prevToken.getLexeme()
					+ " could not use [] operator");
		}
	...
	private void checkVariableName() {
		...
		for (String name : names) {
			if (!isJavaIdentifier(name)) {
				this.reportSyntaxError("Illegal identifier " + name + ",index="
	...
	private void pattern() {
		...
		// this.codeGenerator.onConstant(new PatternToken(sb.toString(),
		// startIndex));
		this.move(true);
	}

	private void reportSyntaxError(String message) {
		throw new MDMPException("Syntax error:" + message + " at "
