...
import com.mdmp.infra.sql.parser.ExpressionLexer;
import com.mdmp.infra.sql.parser.Token;

public class TestEL {
	static Token<?> xxx;

	public static void main(String[] args) {
		ExpressionLexer el = new ExpressionLexer("5+count(if (age>10) then 20 else 50)+cos(60)+sum(age)");
		xxx = el.scan();
		while (xxx != null) {
		...
		// float result = (Float)
		 AviatorEvaluator.execute("5+count(30)+cos(60)");
